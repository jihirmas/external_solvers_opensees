...
class modelLumpedFL:
	def __init__(self, dimension, integration, numIntPts, sameSection, currSec, crossSec, PMMinteraction, includePmaterial, materialP, includeMyMaterial, materialMy, includeMzMaterial, materialMz, includeShearVy, shearVy, includeShearVz, shearVz, includeTorsion, torsion, automaticHingeLength, hingeLengthI, hingeLengthJ):
		self.formulation = 'lumpedFL'
...
class _constants:
	#some constants
	integrationTypes = {"HingeRadau"} #, "HingeRadauTwo", "HingeMidpoint", "HingeEndpoint"}
...
class lumpedFLWidgetPP(QWidget):
	...
	def initialize(self, model, preview):
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::initialize\t\t\t->\tInitialized values. Model before updating GUI:')
			self.printModel()
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::initialize\t\t\t->\tUpdated GUI. Model after updating GUI:')
			self.printModel()
	...
	def updateViewsChangingDimension(self):
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateViewsChangingDimension\t->\tupdating the views after changing in dimension')
	...
	def changedDimension(self):
		...
		if self.sender() is not None:
			...
			if lumpedFLWidgetPP._debug:
				print('lumpedFLWidgetPP::changedDimension\t\t\t->\tDimension set to {}. Model updated:'.format(self.model.dimension))
				self.printModel()
		# Update the views
	...
	def updateSectionNavigator(self):
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateSectionNavigator\t\t\t->\tCalling the function with this model')
			self.printModel()
			print('lumpedFLWidgetPP::updateSectionNavigator\t\t\t->\tSetting current section')
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateSectionNavigator\t\t\t->\tResetting labels')
		self.secNav.resetLabels()
		
		# Update the widget navigator Secion
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateSectionNavigator\t\t\t->\tSetting cross section')
		tooltip = []
		indexSec = []
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateSectionNavigator\t\t\t->\tChecking aggregators')
		showShearVy = False
		showShearVz = False
		showTorsion = False
	...
	def updateGUI(self):
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateGUI\t\t\t->\tModel before updating GUI:')
			self.printModel()
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateGUI\t\t\t->\tsection for update of the gui (from 0 to numIntPts): {} - all same section? {}'.format(sec,self.model.sameSection))
		
		# Cross section Widget
		# Updating the combo of the section
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateGUI\t\t\t->\tUpdating to the following section (index of the xobj): ',self.model.crossSection[sec])
		idx = self.comboSection.findData(self.model.crossSection[sec])
		if idx >= 0:
			self.comboSection.setCurrentIndex(idx)
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateGUI\t\t\t->\tUpdated the combo of the section')
		...
		self.updateSectionNavigator()
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateGUI\t\t\t->\tUpdated the GUI, the model is:')
			self.printModel()
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateGUI\t\t\t->\tCalling update Nav Buttons')
	...
	def getSectionAndUpdateSectionPreviewWidget(self):
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::getSectionAndUpdateSectionPreviewWidget\t\t\t->\tgettingSection and updating PreviewWidget')
	...
	def changedCrossSection(self):
		...
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::changedCrossSection\t\t\t->\tCross section changed. The new model is:')
			self.printModel()
	...
	def updateNavButtons(self):
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::updateNavButtons\t\t->\tCalling update Nav Buttons')
			self.printModel()
			print('lumpedFLWidgetPP::updateNavButtons\t\t->\tself.model.sameSection: {}'.format(self.model.sameSection))
		if self.model.sameSection:
			if lumpedFLWidgetPP._debug:
				print('lumpedFLWidgetPP::updateNavButtons\t\t->\tUsing the same section. Disable buttons')
	...
	def changedPMMinteraction(self):
		# Update model 
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::changedPMMinteraction\t->\tPMM Interaction changed.')
		self.model.PMMinteraction = self.checkBoxPMMinteraction.isChecked()
		# Update the views 
		self.updateViewsChangingPMMinteraction()
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::changedPMMinteraction\t->\tPMM Interaction changed. The new model after update is:')
			self.printModel()
		self.updateImage()
		self.updateSectionNavigator()
	...
	def changedShearInclusion(self):
		if lumpedFLWidgetPP._debug:
			print('lumpedFLWidgetPP::changedShearInclusion\t-\t\t>\tCalled shear InclusionChanged\n')
	...
	def changedTorsionInclusion(self):
		if lumpedFLWidgetPP._debug:
			print('\n\nlumpedFLWidgetPP::changedTorsionInclusion\t\t->\tCalled torsion Inclusion change')
			print('\t\t\t\t\t\t\tSender:', self.sender())
	...
	def changedPhenomenologicalPInclusion(self):
		if lumpedFLWidgetPP._debug:
			print('\n\nlumpedFLWidgetPP::changedPhenomenologicalPInclusion\t\t->\tCalled P Inclusion change')
			print('\t\t\t\t\t\t\tSender:', self.sender())
	...
	def changedPhenomenologicalMyInclusion(self):
		if lumpedFLWidgetPP._debug:
			print('\n\nlumpedFLWidgetPP::changedPhenomenologicalMyInclusion\t\t->\tCalled My Inclusion change')
			print('\t\t\t\t\t\t\tSender:', self.sender())
	...
	def changedPhenomenologicalMzInclusion(self):
		if lumpedFLWidgetPP._debug:
			print('\n\nlumpedFLWidgetPP::changedPhenomenologicalMzInclusion\t\t->\tCalled Mz Inclusion change')
			print('\t\t\t\t\t\t\tSender:', self.sender())
	...
	def updateImage(self):
		...
		fileName = os.path.abspath(os.path.join(folder, "img/lumpedFL"))
		ext = '.png'
		suff = ''
		if self.model.sameSection:
			if (self.model.includeShearVy[0]):
