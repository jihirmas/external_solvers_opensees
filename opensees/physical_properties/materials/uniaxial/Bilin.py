...
def makeXObjectMetaData():
	...
	at_K0.description = (
		...
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	...
	at_LamdaS.description = (
		...
		html_par('Cyclic deterioration parameter for strength deterioration [E_t=Lamda_S*M_y; set Lamda_S = 0 to disable this mode of deterioration]') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	...
	at_LamdaC.description = (
		...
		html_par('Cyclic deterioration parameter for post-capping strength deterioration [E_t=Lamda_C*M_y; set Lamda_C = 0 to disable this mode of deterioration]') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	...
	at_LamdaA.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('Lamda_S')+'<br/>') + 
		html_par('Cyclic deterioration parameter for acceleration reloading stiffness deterioration (is not a deterioration mode for a component with Bilinear hysteretic response) [Input value is required, but not used; set Lamda_A = 0].') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	...
	at_LamdaK.description = (
		...
		html_par('Cyclic deterioration parameter for unloading stiffness deterioration [E_t=Lamda_K*M_y; set Lamda_k = 0 to disable this mode of deterioration]') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	...
	at_thetapNeg.description = (
		...
		html_par('Pre-capping rotation for negative loading direction (often noted as plastic rotation capacity) (positive value)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	...
	at_thetapcNeg.description = (
		...
		html_par('Post-capping rotation for negative loading direction (positive value)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	...
	# use_nFactor
	at_use_nFactor = MpcAttributeMetaData()
	at_use_nFactor.type = MpcAttributeType.Boolean
	at_use_nFactor.name = 'use_nFactor'
	at_use_nFactor.group = 'Non-linear'
	at_use_nFactor.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('use_nFactor')+'<br/>') + 
		html_par('Elastic stiffness amplification factor, mainly for use with concentrated plastic hinge elements (optional, default = 0).') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	
	# nFactor
	at_nFactor = MpcAttributeMetaData()
	at_nFactor.type = MpcAttributeType.Real
	at_nFactor.name = 'nFactor'
	at_nFactor.group = 'Optional parameters'
	at_nFactor.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('nFactor')+'<br/>') + 
		html_par('Elastic stiffness amplification factor, mainly for use with concentrated plastic hinge elements (optional, default = 0).') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Bilinear_Hysteretic_Response_(Bilin_Material)','Bilin Material')+'<br/>') +
		html_end()
		)
	at_nFactor.setDefault(0.0)
	...
	xom.addAttribute(at_use_nFactor)
	xom.addAttribute(at_nFactor)
	
	# nFactor-dep
	xom.setVisibilityDependency(at_use_nFactor, at_nFactor)
...
def writeTcl(pinfo):
	...
	use_nFactor_at = xobj.getAttribute('use_nFactor')
	if(use_nFactor_at is None):
		raise Exception('Error: cannot find "use_nFactor" attribute')
	use_nFactor = use_nFactor_at.boolean
	if use_nFactor:
		nFactor_at = xobj.getAttribute('nFactor')
		if(nFactor_at is None):
			raise Exception('Error: cannot find "nFactor" attribute')
		nFactor = nFactor_at.real
		
		sopt += '{}'.format(nFactor)
	
	
	str_tcl = '{}uniaxialMaterial Bilin {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}\n'.format(pinfo.indent, tag,
		K0.value, as_Plus, as_Neg, My_Plus.value, My_Neg.value, Lamda_S, Lamda_C, Lamda_A, Lamda_K, c_S, c_C, c_A, c_K, theta_p_Plus, theta_p_Neg,
		theta_pc_Plus, theta_pc_Neg, Res_Pos.value, Res_Neg.value, theta_u_Plus, theta_u_Neg, D_Plus, D_Neg, sopt)
