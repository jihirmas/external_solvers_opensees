...
def makeXObjectMetaData():
	...
	at_sce.description = (
		...
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/AxialSpHD_Material','AxialSpHD Material')+'<br/>') +
		html_end()
		)
	...
	# bth
	at_bth = MpcAttributeMetaData()
	at_bth.type = MpcAttributeType.Real
	at_bth.name = 'bth'
	at_bth.group = 'Optional parameters'
	at_bth.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('bth')+'<br/>') + 
		html_par('reduction rate for tensile hardening (see note 1)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/AxialSpHD_Material','AxialSpHD Material')+'<br/>') +
		html_end()
		)
	...
	# ath
	at_ath = MpcAttributeMetaData()
	at_ath.type = MpcAttributeType.QuantityScalar
	at_ath.name = 'ath'
	at_ath.group = 'Non-linear'
	at_ath.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('ath')+'<br/>') + 
		html_par('hardening strain ratio to yield strain (see note 1)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/AxialSpHD_Material','AxialSpHD Material')+'<br/>') +
		html_end()
		)
...
def writeTcl(pinfo):
	...
	if Optional:
		...
		bth_at = xobj.getAttribute('bth')
		if(bth_at is None):
			raise Exception('Error: cannot find "bth" attribute')
		bth = bth_at.real
		...
		ath_at = xobj.getAttribute('ath')
		if(ath_at is None):
			raise Exception('Error: cannot find "ath" attribute')
		ath = ath_at.real
		
		sopt += '{} {} {} {} {} {}'.format(bte, bty, bth, bcy, fcr, ath)
	
	str_tcl = '{}uniaxialMaterial AxialSpHD {} {} {} {} {}\n'.format(pinfo.indent, tag, sce.value, fty.value, fcy.value, sopt)
