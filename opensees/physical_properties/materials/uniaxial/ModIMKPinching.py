...
def makeXObjectMetaData():
	...
	at_K0.description = (
		...
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Pinched_Hysteretic_Response_(ModIMKPinching_Material)','ModIMKPinching Material')+'<br/>') +
		html_end()
		)
	...
	# FprPos
	at_FprPos = MpcAttributeMetaData()
	at_FprPos.type = MpcAttributeType.Real
	at_FprPos.name = 'FprPos'
	at_FprPos.group = 'Non-linear'
	at_FprPos.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('FprPos')+'<br/>') + 
		html_par('Ratio of the force at which reloading begins to force corresponding to the maximum historic deformation demand (positive loading direction)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Pinched_Hysteretic_Response_(ModIMKPinching_Material)','ModIMKPinching Material')+'<br/>') +
		html_end()
		)

	# FprNeg
	at_FprNeg = MpcAttributeMetaData()
	at_FprNeg.type = MpcAttributeType.Real
	at_FprNeg.name = 'FprNeg'
	at_FprNeg.group = 'Non-linear'
	at_FprNeg.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('FprNeg')+'<br/>') + 
		html_par('Ratio of the force at which reloading begins to force corresponding to the absolute maximum historic deformation demand (negative loading direction)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Pinched_Hysteretic_Response_(ModIMKPinching_Material)','ModIMKPinching Material')+'<br/>') +
		html_end()
		)
	
	# APinch
	at_APinch = MpcAttributeMetaData()
	at_APinch.type = MpcAttributeType.Real
	at_APinch.name = 'APinch'
	at_APinch.group = 'Non-linear'
	at_APinch.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('A_Pinch')+'<br/>') + 
		html_par('Ratio of reloading stiffness') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Modified_Ibarra-Medina-Krawinkler_Deterioration_Model_with_Pinched_Hysteretic_Response_(ModIMKPinching_Material)','ModIMKPinching Material')+'<br/>') +
		html_end()
		)
	...
	xom.addAttribute(at_FprPos)
	xom.addAttribute(at_FprNeg)
	xom.addAttribute(at_APinch)
...
def writeTcl(pinfo):
	...
	#uniaxialMaterial ModIMKPinching $matTag $K0 $as_Plus $as_Neg $My_Plus $My_Neg $FprPos $FprNeg $A_pinch
	#$Lamda_S $Lamda_C $Lamda_A $Lamda_K $c_S $c_C $c_A $c_K $theta_p_Plus $theta_p_Neg $theta_pc_Plus
	...
	FprPos_at = xobj.getAttribute('FprPos')
	if(FprPos_at is None):
		raise Exception('Error: cannot find "FprPos" attribute')
	FprPos = FprPos_at.real
	
	FprNeg_at = xobj.getAttribute('FprNeg')
	if(FprNeg_at is None):
		raise Exception('Error: cannot find "FprNeg" attribute')
	FprNeg = FprNeg_at.real
	
	A_pinch_at = xobj.getAttribute('APinch')
	if(A_pinch_at is None):
		raise Exception('Error: cannot find "APinch" attribute')
	A_pinch = A_pinch_at.real
	...
	str_tcl = '{}uniaxialMaterial ModIMKPinching {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {} {}\n'.format(
			pinfo.indent, tag, K0.value, as_Plus, as_Neg, My_Plus.value, My_Neg.value, FprPos, FprNeg, A_pinch, Lamda_S, Lamda_C, Lamda_A, Lamda_K, c_S, c_C,
			c_A, c_K, theta_p_Plus, theta_p_Neg, theta_pc_Plus, theta_pc_Neg, Res_Pos.value, Res_Neg.value, theta_u_Plus, theta_u_Neg, D_Plus, D_Neg)
