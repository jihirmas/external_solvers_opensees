...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Laplace Random Variable')+'<br/>') +
		html_end()
		)
	...
	at_useParameters.description = (
		...
		html_par('the parameters u and k of the distribution') +
	...
	at_alpha.description = (
		...
		html_par('the alpha parameter of distribution (alpha is the location parameter)') +
		html_par('<u>Please note</u> that if parameters are specified, mean and stdv are ignored') +
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Laplace Random Variable')+'<br/>') +
		html_end()
		)
	...
	at_beta.name = 'beta'
	at_beta.group = '-parameters'
	at_beta.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('beta')+'<br/>') +
		html_par('the beta of the distribution (beta is the inverse of the scale parameter)') +
		html_par('<u>Please note</u> that if parameters are specified, mean and stdv are ignored') +
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Laplace Random Variable')+'<br/>') +
		html_end()
		)
	...
	xom.setVisibilityDependency(at_useParameters, at_beta)
...
def writeTcl(pinfo):
	...
	#randomVariable $tag laplace <-mean $mean -stdv $stdv> <-parameters $lambda $x0>
	...
	if useParameters:
		...
		beta = _get_xobj_attribute(xobj, 'beta').real
	else:
		#get mean
	...
	if useParameters:
		str_tcl = '{}randomVariable {} laplace -parameters {} {}{}\n'.format(pinfo.indent, tag, alpha, beta, sopt)
	else:
		str_tcl = '{}randomVariable {} laplace -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
