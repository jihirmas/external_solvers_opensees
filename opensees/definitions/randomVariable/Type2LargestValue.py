...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Type2LargestValue Random Variable')+'<br/>') +
		html_end()
		)
	...
	at_k.name = 'k'
	at_k.group = '-parameters'
	at_k.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('k')+'<br/>') +
		html_par('the k of the distribution (k is the scale parameter)') +
		html_par('<u>Please note</u> that if parameters are specified, mean and stdv are ignored') +
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Type2LargestValue Random Variable')+'<br/>') +
		html_end()
		)
	...
	xom.setVisibilityDependency(at_useParameters, at_k)
...
def writeTcl(pinfo):
	...
	#randomVariable $tag type2LargestValue <-mean $mean -stdv $stdv> <-parameters $lambda $x0>
	...
	if useParameters:
		str_tcl = '{}randomVariable {} type2LargestValue -parameters {} {}{}\n'.format(pinfo.indent, tag, u, k, sopt)
	else:
		str_tcl = '{}randomVariable {} type2LargestValue -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
