...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Type1LargestValue Random Variable')+'<br/>') +
		html_end()
		)
...
def writeTcl(pinfo):
	...
	#randomVariable $tag type1LargestValue <-mean $mean -stdv $stdv> <-parameters $lambda $x0>
	...
	if useParameters:
		str_tcl = '{}randomVariable {} type1LargestValue -parameters {} {}{}\n'.format(pinfo.indent, tag, u, alpha, sopt)
	else:
		str_tcl = '{}randomVariable {} type1LargestValue -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
