...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','ChiSquare Random Variable')+'<br/>') +
		html_end()
		)
	...
	at_useParameters.description = (
		...
		html_par('the parameter nu of the distribution') +
	...
	at_nu.name = 'kappa'
	at_nu.group = '-parameters'
	at_nu.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('kappa')+'<br/>') +
		html_par('the nu of the distribution (nu is the degree of freedom della distribuzione)') +
		html_par('<u>Please note</u> that if parameters are specified, mean and stdv are ignored') +
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','ChiSquare Random Variable')+'<br/>') +
		html_end()
		)
	...
	# nu - dep
	xom.setVisibilityDependency(at_useParameters, at_nu)
...
def writeTcl(pinfo):
	...
	#randomVariable $tag chiSquare <-mean $mean -stdv $stdv> <-parameters $kappa $lambda>
	...
	if useParameters:
		nu = _get_xobj_attribute(xobj, 'nu').real
	else:
		#get mean
	...
	if useParameters:
		str_tcl = '{}randomVariable {} chiSquare -parameters {}{}\n'.format(pinfo.indent, tag, nu, sopt)
	else:
		str_tcl = '{}randomVariable {} chiSquare -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
