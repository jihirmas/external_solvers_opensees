...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Exponential Random Variable')+'<br/>') +
		html_end()
		)
	...
	at_useParameters.description = (
		...
		html_par('the parameters lambda of the distribution') +
...
def writeTcl(pinfo):
	...
	#randomVariable $tag exponential <-mean $mean -stdv $stdv> <-parameters $lambda $x0>
	...
	if useParameters:
		str_tcl = '{}randomVariable {} exponential -parameters {} {}\n'.format(pinfo.indent, tag, lamda, sopt)
	else:
		str_tcl = '{}randomVariable {} exponential -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
