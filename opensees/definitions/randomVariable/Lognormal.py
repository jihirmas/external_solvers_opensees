...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Lognormal Random Variable')+'<br/>') +
		html_end()
		)
	...
	at_lambda.description = (
		...
		html_par('the lambda of the distribution (lambda is equal to the log of scale parameter)') +
		html_par('<u>Please note</u> that if parameters are specified, mean and stdv are ignored') +
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Lognormal Random Variable')+'<br/>') +
		html_end()
		)
	...
	# zeta
	at_zeta = MpcAttributeMetaData()
	at_zeta.type = MpcAttributeType.Real
	at_zeta.name = 'zeta'
	at_zeta.group = '-parameters'
	at_zeta.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('zeta')+'<br/>') +
		html_par('the zeta of the distribution (zeta is equal to the shape parameter)') +
		html_par('<u>Please note</u> that if parameters are specified, mean and stdv are ignored') +
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Lognormal Random Variable')+'<br/>') +
		html_end()
		)
	...
	xom.setVisibilityDependency(at_useParameters, at_zeta)
...
def writeTcl(pinfo):
	...
	#randomVariable $tag lognormal <-mean $mean -stdv $stdv> <-parameters $lambda $zeta>
	...
	if useParameters:
		...
		zeta = _get_xobj_attribute(xobj, 'zeta'). real
	else:
		#get mean
	...
	if useParameters:
		str_tcl = '{}randomVariable {} lognormal -parameters {} {}{}\n'.format(pinfo.indent, tag, lamda, zeta, sopt)
	else:
		str_tcl = '{}randomVariable {} lognormal -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
