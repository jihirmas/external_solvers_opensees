...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','ShiftedExponential Random Variable')+'<br/>') +
		html_end()
		)
	...
	at_useParameters.description = (
		...
		html_par('the parameters lambda and x0 of the distribution') +
	...
	at_lambda.description = (
		...
		html_par('the lambda of the distribution (lambda is the rate parameter)') +
		html_par('<u>Please note</u> that if parameters are specified, mean and stdv are ignored') +
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','ShiftedExponential Random Variable')+'<br/>') +
		html_end()
		)
...
def writeTcl(pinfo):
	...
	#randomVariable $tag shiftedExponential <-mean $mean -stdv $stdv> <-parameters $lambda $x0>
	...
	if useParameters:
		str_tcl = '{}randomVariable {} shiftedExponential -parameters {} {}{}\n'.format(pinfo.indent, tag, lamda, x0, sopt)
	else:
		str_tcl = '{}randomVariable {} shiftedExponential -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
