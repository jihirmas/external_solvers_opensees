...
def makeXObjectMetaData():
	...
	at_mean.description = (
		...
		html_par(html_href('https://opensees.berkeley.edu/wiki/index.php/Sensitivity_Command_Manual','Weibull Random Variable')+'<br/>') +
		html_end()
		)
...
def writeTcl(pinfo):
	...
	#randomVariable $tag weibull <-mean $mean -stdv $stdv> <-parameters $lambda $x0>
	...
	if useParameters:
		str_tcl = '{}randomVariable {} weibull -parameters {} {}{}\n'.format(pinfo.indent, tag, u, k, sopt)
	else:
		str_tcl = '{}randomVariable {} weibull -mean {} -stdv {}{}\n'.format(pinfo.indent, tag, mu, sigma, sopt)
