...
def makeXObjectMetaData():
	...
	at_tStart.description = (
		...
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Pulse_TimeSeries','Pulse TimeSeries')+'<br/>') +
		html_end()
		)
	...
	at_period.description = (
		...
		html_par('characteristic period of pulse') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Pulse_TimeSeries','Pulse TimeSeries')+'<br/>') +
		html_end()
		)
	...
	at_width.type = MpcAttributeType.Boolean
	at_width.name = '-width'
	at_width.group = 'Group'
	at_width.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('-width')+'<br/>') + 
		html_par('pulse width as a fraction of the period (optional, default = 0.5)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Pulse_TimeSeries','Pulse TimeSeries')+'<br/>') +
		html_end()
		)
	
	# pulseWidth
	at_pulseWidth = MpcAttributeMetaData()
	at_pulseWidth.type = MpcAttributeType.Real
	at_pulseWidth.name = 'pulseWidth'
	at_pulseWidth.group = '-width'
	at_pulseWidth.description = (
		html_par(html_begin()) +
		html_par(html_boldtext('pulseWidth')+'<br/>') + 
		html_par('pulse width as a fraction of the period (optional, default = 0.5)') +
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/Pulse_TimeSeries','Pulse TimeSeries')+'<br/>') +
		html_end()
		)
	at_pulseWidth.setDefault(0.5)
	...
	xom.addAttribute(at_pulseWidth)
	...
	# width-dep
	xom.setVisibilityDependency(at_width, at_pulseWidth)
...
def evaluateFunctionAttribute(xobj):
	...
	if(xobj.name != 'Pulse'):
		print('Error: invalid xobj type, expected "Pulse", given "{}"'.format(xobj.name))
		return PyMpc.Math.mat()
	...
	if(width_at is None):
		print('Error: cannot find "-width" attribute\n')
		return PyMpc.Math.mat()
	
	pulseWidth_at = xobj.getAttribute('pulseWidth')
	if(pulseWidth_at is None):
		print('Error: cannot find "pulseWidth" attribute\n')
		return PyMpc.Math.mat()
	...
	Dt = tEnd - tStart
	n = int(math.ceil(Dt/period/pulseWidth))
	...
	for i in range(2, pnt+2):
		...
		iy = 0.0
		temp = (ix+shift-tStart)/period
		k = temp - math.floor(temp)
		if k < pulseWidth:
			iy = cFactor
		elif k < 1.0:
			iy = 0.0
...
def writeTcl(pinfo):
	...
	width = width_at.boolean
	if width:
		pulseWidth_at = xobj.getAttribute('pulseWidth')
		if(pulseWidth_at is None):
			raise Exception('Error: cannot find "pulseWidth" attribute')
		pulseWidth = pulseWidth_at.real
		sopt += ' -width {}'.format(pulseWidth)
