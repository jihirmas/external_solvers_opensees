...
def __processEdgeDistributedLK (pinfo, doc, all_geom, alpha_T, C_T, alpha, C, is_partitioned, process_id, process_block_count, indent):
	...
	for geom, subset in all_geom.items():
		...
		for edge_id in subset.edges:
			domain = mesh_of_geom.edges[edge_id]
				...
				for gauss_point in elem.integrationRule.integrationPoints:
					...
					for i in range(n):
						...
						if not inode.id in extra_node_data:
							...
							# pinfo.updateModelBuilder(node_ndm, node_ndf)
							if node_ndm == 3:
								raise Exception('Error: dimensions 3D for condition 2D')
		...
		if not is_partitioned:
				pinfo.out_file.write('\n{}{}{}'.format(pinfo.indent, indent, '# distributedLK 2D'))
		...
		for source_node_id, node_data in extra_node_data.items():
			...
			dir = '{}'.format('1 2')
			strMat = '{} {}'.format(mat_tag-1, mat_tag)
			...
			if extra_node_data[source_node_id].N2 is not None: # then source = 2,3 (UP)
...
def writeTcl_mpConstraints(pinfo):
	...
	if is_partitioned:
		...
		for process_id in range(pinfo.process_count):
			...
			process_block_count = __processEdgeDistributedLK (pinfo, doc, all_geom, alpha_T, C_T, alpha, C, is_partitioned, process_id, process_block_count, pinfo.tabIndent)
		pinfo.out_file.write('\n')
	else:
		__processEdgeDistributedLK (pinfo, doc, all_geom, alpha_T, C_T, alpha, C, is_partitioned, 0, 0, pinfo.indent)