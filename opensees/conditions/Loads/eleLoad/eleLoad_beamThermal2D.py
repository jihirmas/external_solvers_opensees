...
def makeXObjectMetaData():
	...
	xom.name = 'eleLoad_beamThermal2D'
...
def __processEleLoadBeamThemal2D(pinfo, xobj, user, T, y, tag, eleTag, indent):
	if user:
		if(len(T)!=len(y)):
...
def writeTcl_eleLoad(pinfo, xobj):
	...
	# eleLoad -ele $eleTag -type -shellThermal $T1 $y1 $T2 $Y2 <$T3 $y3 ... $T9 $Y9>
	...
	if is_partitioned:
		...
		for process_id in range(pinfo.process_count):
			...
			for geom, item in all_geom.items():
				...
				if eleTag:
					__processEleLoadBeamThemal2D(pinfo, xobj, user, T, y, tag, eleTag, pinfo.tabIndent)
			if is_partitioned :
				if first_done:
		...
		for geom, item in all_geom.items():
			...
			__processEleLoadBeamThemal2D(pinfo, xobj, user, T, y, tag, eleTag, pinfo.indent)