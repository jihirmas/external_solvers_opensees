...
def makeXObjectMetaData():
	...
	at_gTol.description = (
		...
		html_par(html_href('http://opensees.berkeley.edu/wiki/index.php/SimpleContact3D','SimpleContact3D')+'<br/>') +
		html_end()
		)
...
def writeTcl(pinfo):
	...
	if(nameMaterial!='ContactMaterial3D'):
		raise Exception('Error: material must be "ContactMaterial3D" and not "{}"'.format(nameMaterial))
	
	# NODE
	nstr = ''
	node_vect = []
	for node in elem.nodes:
		node_vect.append(node)
		nstr += ' {}'.format(node.id)
	...
	# stuff to write
	strNode='{}{} {} {} {} {}\n'.format(pinfo.indent,'\n#Lagrange multiplier node\n# node', 'tag', 'x', 'y', 'z')
	# Lagrange multiplier node needs 3 ndm , 3 ndf
